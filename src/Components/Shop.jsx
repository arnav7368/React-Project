...
export default function Shop() {
  ...
  let [filterdata2, setFilterdata2] = useState([]);
  ...
  function priceFilter(e) {
    ...
    if (search)
      setProducts(
        filterdata2.filter((x) => x.finalprice >= min && x.finalprice <= max)
      );
    else if(filterdata1.length)
      setProducts(
        filterdata1.filter((x) => x.finalprice >= min && x.finalprice <= max)
      );
    else
    setProducts(
        ProductStateData.filter((x) => x.finalprice >= min && x.finalprice <= max)
      );
  }
  ...
  function sortFilter(e) {
    ...
    if (value === "1") products.sort().reverse();
    else if (value === "2")
      products.sort((x, y) => x.finalprice - y.finalprice);
    else products.sort((x, y) => y.finalprice - x.finalprice);
  ...
  function filterData(mc, sc, br) {
    let data = [];
    if (mc === null && sc === null && br === null) data = ProductStateData;
      ...
      data = ProductStateData.filter(
        (x) => x.brand === br && x.subcategory === sc && x.maincategory === mc
      );
    setProducts(data);
    setFilterdata1(data);
  }
  ...
  return (
    <>
      <Breadcurm title="Shop" />

      <div className="container-fluid my-3 ">
        <div className="row">
          <div className="col-lg-2 col-md-3">
            <div className="list-group rounded-box">
              <Link
                to="#"
              ...
              <button
                onClick={() => getCategoryFilters(null, sc, br)}
              ...
              {maincategories.map((item, index) => {
                return (
                  <button
                    onClick={() => getCategoryFilters(item.name, sc, br)}
                    key={index}
                    className={`list-group-item list-group-item-action ${
            ...
            <div className="list-group my-3 rounded-box">
              <Link
                to="#"
                className="list-group-item list-group-item-action active"
                aria-current="true"
              ...
              <button
                onClick={() => getCategoryFilters(mc, null, br)}
                className={`list-group-item list-group-item-action ${
              ...
              {subcategories.map((item, index) => {
                return (
                  <button
                    onClick={() => getCategoryFilters(mc, item.name, br)}
                    key={index}
                    className={`list-group-item list-group-item-action ${
            ...
            <div className="list-group my-3 rounded-box">
              ...
              <button
                onClick={() => getCategoryFilters(mc, sc, null)}
                className={`list-group-item list-group-item-action ${
              ...
              {brands.map((item, index) => {
                return (
                  <button
                    onClick={() => getCategoryFilters(mc, sc, item.name)}
                    key={index}
                    className={`list-group-item list-group-item-action ${
            ...
            <form onSubmit={priceFilter} className="rounded-box">
              <div className="my-3 ">
                <input
                  type="number"
                  name="min"
          ...
          <div className="col-lg-10 col-md-9">
            <div className="row mb-2">
              ...
              <div className="col-lg-3 col-md-4 col-12 rounded-box">
                <select
                  name="sort"
                  onChange={sortFilter}
                  className="form-control"
